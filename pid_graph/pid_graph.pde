 ...
 **********************************************/
int windowWidth = 1800;      // set the size of the 
int windowHeight = 600;     // form

float InScaleMin = -450;       // set the Y-Axis Min
float InScaleMax = 450;    // and Max for both
float OutScaleMin = -255;      // the top and 
float OutScaleMax = 255;    // bottom trends


int windowSpan = 18000;    // number of mS into the past you want to display
...
void drawGraph()
{
  ...
  for(int i=0;i<7;i++)
  {
    ...
    text(str((InScaleMax-InScaleMin)/6*(float)(6-i)+InScaleMin),ioRight+5,inputTop+i*interval+4);
  ...
  for(int i=0;i<7;i++)
  {
    ...
    text(str((OutScaleMax-OutScaleMin)/6*(float)(6-i)+OutScaleMin),ioRight+5,outputTop+i*interval+4);
  }
...
void serialEvent(Serial myPort)
{
  ...
  if (s.length == 22)
  {
    Setpoint = float(s[9]);           // * pull the information
    Input = float(s[12]);              //   we need out of the
    Output = float(s[20]);             //   string and put it
    SPLabel.setValue(s[9]);           //   where it's needed
    InLabel.setValue(s[12]);           //
    OutLabel.setValue(trim(s[20]));    //
    ...
    {                                 //   since we sent values to 
      SPField.setText(trim(s[9]));    //   the arduino,  take the
      InField.setText(trim(s[12]));    //   current values and put
      OutField.setText(trim(s[20]));   //   them into the input fields
